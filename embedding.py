    ...
    def _init_deep_model(self):
        ...
        try:
            ...
            if "dinov2" in self.model_name:
                # Use direct PyTorch loading for DINOv2
                import torchvision.models as models

                # Map model name to the appropriate function
                model_mapping = {
                ...
                model_func_name = model_mapping.get(self.model_name, "dinov2_vits14")
                model_func = getattr(models, model_func_name, None)

                if model_func is not None:
                    self.model = model_func(pretrained=True).to(self.device)
                    self.model.eval()
                    self.extractor = None  # No need for separate extractor
                    self.logger.info(f"DINOv2 model loaded successfully on {self.device}")
                else:
                    raise ImportError(f"Could not find model function {model_func_name}")
            else:
                # Use HuggingFace Transformers for other models
    ...
    def _extract_deep_features(self, image: np.ndarray) -> np.ndarray:
        ...
        try:
            if "dinov2" in self.model_name:
                ...
                # Convert from BGR to RGB and apply transform
                if len(image.shape) == 3 and image.shape[2] == 3:
                    rgb_image = image[..., ::-1]  # BGR to RGB
                else:
                    # Repeat grayscale to 3 channels
                    rgb_image = np.repeat(image[..., np.newaxis], 3, axis=2) if len(image.shape) == 2 else image

                tensor_image = transform(rgb_image).unsqueeze(0).to(self.device)

                # Extract features
                with torch.no_grad():
                    features = self.model(tensor_image)

                # For DINOv2, normalize the output features
                features = F.normalize(features, dim=1)
                return features.cpu().numpy().flatten()
            else:
                # Process for HuggingFace Transformers models
